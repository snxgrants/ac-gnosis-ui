...
import { getChainInfo, getTxServiceUrl } from '../config';
...
const useIframeMessageHandler = (
  ...
  useEffect(() => {
    const handleIframeMessage = (
      ...
      trackEvent({
        ...SAFE_APPS_EVENTS.LEGACY_API_CALL,
        label: `${getSafeAppName(selectedApp)}-${messageId}`,
      });
      ...
      switch (messageId) {
          ...
          const safeInfoMessage = {
            ...
            data: {
              ...
              network: getLegacyChainName(
                chainName,
                chainId
              ).toLowerCase() as LowercaseNetworks,
        ...
        default: {
          console.error(
            `ThirdPartyApp: A message was received with an unknown message id ${messageId}.`
          );
          break;
        }
      }
    ...
    const onIframeMessage = async (
      ...
      if (
        ...
        message.origin === THIRD_PARTY_COOKIES_CHECK_URL
      ) {
        return;
      }
      ...
      if (!selectedApp?.url.includes(message.origin)) {
        console.error(
          `ThirdPartyApp: A message was received from an unknown origin ${message.origin}`
        );
        return;
      }
      handleIframeMessage(
